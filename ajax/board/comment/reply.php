...
if(!$icmt) {
	...
	$result = Db::query("UPDATE comment SET content = '$content' WHERE icmt = $icmt AND iuser = $iuser");
}
...
array_push($array, array('result'=>$result, 'content'=>Utils::replaceNewLine($row['content']), 'nm'=>$row['nm'], 'rdt'=>$row['rdt'], 'icmt'=>$row['icmt'], 'iuser'=>$row['iuser']));
